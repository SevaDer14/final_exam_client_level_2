...
const Courses = ({category}) => {
  ...
  if (category) {
    courcesInCategory = courses.filter((course) => course.category === category)
  }
  ...
  const listOfCources = courcesInCategory.map((course) => (
    <div data-cy='course-container'>
      ...
      <p data-cy='course-category' className='capitalized'><span className='bold'>Category: </span>{course.category}</p>
